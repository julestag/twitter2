...
if(isset($_GET['print'])){
    $requete = $bdd->query("SELECT tweet.content,user.at_user_name,user.username,profile_picture,COUNT(likes.id_user) as likes_count,tweet.id as twid FROM tweet INNER JOIN user ON tweet.id_user = user.id LEFT JOIN likes ON tweet.id = likes.id_tweet WHERE id_quoted_tweet IS NULL GROUP BY tweet.id ORDER BY tweet.time DESC");
...
if(isset($_POST['quoted_tweet'])){
    ...
    $query = $bdd -> prepare("SELECT tweet.content,user.at_user_name,user.username,profile_picture,COUNT(likes.id_user) as likes_count,id_quoted_tweet,tweet.id as twid FROM tweet INNER JOIN user ON tweet.id_user = user.id LEFT JOIN likes ON tweet.id = likes.id_tweet WHERE  id_quoted_tweet LIKE '$id_tweet_quoted' GROUP BY tweet.id ORDER BY tweet.time ASC");
    $query -> execute();
    $tweet_response = $query->fetchAll(PDO::FETCH_ASSOC);
   // $result = array_merge($tweets,$tweet_response);
...
if(array_key_exists('retweet',$_POST)){
    ...
    foreach($result_id_tweet as $rows){
        ...
        $query2 = $bdd -> prepare("SELECT tweet.content,user.at_user_name,user.username,profile_picture,COUNT(likes.id_user) as likes_count,tweet.id as twid FROM tweet INNER JOIN user ON tweet.id_user = user.id LEFT JOIN likes ON tweet.id = likes.id_tweet WHERE tweet.id LIKE '$id_tweet_retweet' GROUP BY tweet.id ORDER BY tweet.time DESC");
        $query2 -> execute();
